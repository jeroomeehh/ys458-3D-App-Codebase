...
$(document).ready(function(){
	...
	$.getJSON('./index.php/apiGetData', function(jsonModel3D) {
	 ...
	 //AJAX service request to get the main text data from the json data
	 $.getJSON('./index.php/apiGetPageData', function(jsonObj) {
		...
		$('#x3dCameraTitle').html(jsonObj[5].title);
		$('#x3dCameraSubtitle').html('<p>' + jsonObj[5].description + '<p');

		//Get render contents data
		$('#x3dRenderTitle').html(jsonObj[6].title);
		$('#x3dRenderSubtitle').html('<p>' + jsonObj[6].description + '<p');

		//Get animation contents data
		$('#x3dAnimationTitle').html(jsonObj[7].title);
		$('#x3dAnimationSubtitle').html('<p>' + jsonObj[7].description + '<p');
